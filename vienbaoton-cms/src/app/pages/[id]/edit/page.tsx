...
export default function EditPagePage({ params }: PageProps) {
  ...
  async function fetchPage(pageId: string) {
    try {
      ...
      if (!page) {
        setMessage("Không tìm thấy trang");
        setLoading(false);
        return;
      }
      setTitle(page.title || "");
      ...
      setHtml(page.html || "");
      setTags((page.tags as Array<{ name: string }>)?.map((t) => t.name).join(", ") || "");
      setFeatureImage(page.feature_image || "");
      setStatus(page.status || "draft");
    } catch {
      setMessage("Lỗi tải trang");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="grid grid-cols-[1fr_320px] gap-6">
      <form onSubmit={handleSubmit}>
        <div className="section-card mb-0">
          <div className="section-title">Chỉnh sửa trang</div>
